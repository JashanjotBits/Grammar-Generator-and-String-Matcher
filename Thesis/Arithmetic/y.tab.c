...
#define operator 258
#define number 259
#define OPENBRACKET 260
#define CLOSEBRACKET 261
...
{
  "$end", "error", "$undefined", "operator", "number", "OPENBRACKET",
  "CLOSEBRACKET", "SEMICOLON", "$accept", "input", "exp", YY_NULLPTR
};
#endif
...
#line 117 "arith.y" /* yacc.c:1646  */
    { printf("Enter the name of the files for storing inorder and preorder traversal rep\n");scanf("%s %s",in,pre);i=fopen(in,"w");printtreetofileIn((yyvsp[-1].a_tree),i);p=fopen(pre,"w");printtreetofilePre((yyvsp[-1].a_tree),p);exit(0); }
...
#line 126 "arith.y" /* yacc.c:1646  */
    {
      ...
      (yyval.a_tree) =make_operator((yyvsp[-2].a_tree),(yyvsp[0].a_tree),(yyvsp[-1].an_op));
...
#line 135 "arith.y" /* yacc.c:1646  */
    {
      (yyval.a_tree)=make_operator((yyvsp[-1].a_tree),NULL,' '); 
      //printtree($$,0);      
    }
